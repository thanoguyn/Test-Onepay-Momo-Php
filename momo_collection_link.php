<?php
/**
 * TÀI LIỆU TÍCH HỢP MOMO: COLLECTION LINK (LINK THU HỘ)
...
// ---------------------------
// 1. INIT COLLECTION LINK (AJAX)[cite: 12]
// ---------------------------
if ($action === 'init') {
    ...
    $requestId = time() . "";
    $requestType = "payWithMethod"; // Đặc trưng của Collection Link[cite: 12]
    $autoCapture = true; // Mặc định capture ngay[cite: 12]
    
    // Hash format cho Collection Link[cite: 12]
...
// ---------------------------
// 2. QUERY TRANSACTION (AJAX)[cite: 12]
// ---------------------------
if ($action === 'query') {
    header('Content-Type: application/json');
...
if ($action === 'verify') {
    ...
    // Chuỗi Hash cố định cho Return/IPN của MoMo
...
if ($action === 'return' || $action === 'ipn') {
    ...
    <body class="bg-gray-100 py-10">
        <div class="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow-xl">
            <h2 class="text-2xl font-bold mb-6 text-gray-800"><?php echo $pageTitle; ?> - Collection Link</h2>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2">
                    <p class="text-sm text-gray-500 mb-4 bg-blue-50 p-3 rounded border border-blue-100">
                        Nhập <b>Secret Key</b> để tự xác thực chữ ký (Access Key và các tham số khác lấy trực tiếp từ URL bên trái).
                    </p>
        ...
        <script>
            ...
            window.onload = function() {
                let conf = JSON.parse(localStorage.getItem('momoCollectionConfig') || '{}');
...
<head>
    ...
    <title>MoMo Collection Link Sandbox</title>
    <script src="https://cdn.tailwindcss.com"></script>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-7 space-y-6">
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden border border-gray-200">
                <div class="bg-pink-600 text-white px-6 py-4 font-bold text-lg">
                    Khởi tạo thanh toán Collection Link (payWithMethod)
                </div>
                <div class="p-6">
                    <form id="initForm" onsubmit="submitInitAjax(event)">
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Payment Endpoint URL (API Create)</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">RedirectUrl (Return URL)</label>
                                <input type="text" id="redirectUrl" name="redirectUrl" class="config-item w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-pink-500" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">IpnUrl (Webhook)</label>
                                <input type="text" id="ipnUrl" name="ipnUrl" class="config-item w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-pink-500" />
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-2 justify-between items-center bg-gray-50 p-3 rounded border">
                            <div class="flex gap-2">
                                ...
                                <button type="button" id="btnCopyPayUrl" class="hidden bg-blue-500 hover:bg-blue-600 text-white px-5 py-2 rounded font-medium shadow transition" onclick="copyPayUrl()">Copy Link Chuyển Hướng</button>
                            </div>
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden border border-gray-200">
                ...
                <div class="p-6">
                    <div class="mb-4">
                        <label class="block text-sm font-bold text-gray-700 mb-1">Dán URL Return/Callback hoặc JSON phản hồi vào đây</label>
                    ...
                    <div class="mb-4">
                        <label class="block text-sm font-bold text-gray-700 mb-1">Chuỗi Hash đầu vào (Định dạng cứng của MoMo)</label>
...
<script>
    ...
    const defaultConfig = {
        ...
        amount: "50000",
        orderInfo: "Thanh toán qua MoMo Collection Link",
        redirectUrl: baseUrl + "?action=return",
        ipnUrl: baseUrl + "?action=ipn",
        endpoint_pay: "https://test-payment.momo.vn/v2/gateway/api/create", // Mặc định URL Collection[cite: 12]
    ...
    function loadConfig() {
        let stored = localStorage.getItem('momoCollectionConfig');
        let config = stored ? JSON.parse(stored) : defaultConfig;
        
        if (!stored) localStorage.setItem('momoCollectionConfig', JSON.stringify(config));
        
        if (!config.redirectUrl || config.redirectUrl.indexOf('?action=return') === -1) config.redirectUrl = baseUrl + "?action=return";
    ...
    function saveConfig() {
        ...
        localStorage.setItem('momoCollectionConfig', JSON.stringify(config));
    ...
    function resetConfig() {
        if(confirm("Xác nhận reset toàn bộ thông số về mặc định?")) {
            localStorage.removeItem('momoCollectionConfig');
            loadConfig();
    ...
    function parseUrlAndCalculate() {
        ...
        try {
            ...
            // Format bắt buộc từ Document MoMo
    ...
    async function submitInitAjax(e) {
        ...
        try {
            ...
            if (json.payUrl) {
                ...
                let btnCopy = document.getElementById('btnCopyPayUrl');
                btnCopy.dataset.url = json.payUrl;
    ...
    function buildMockUrl(type, isSuccess) {
        ...
        // Chuỗi format fix cứng
        let rawHash = "partnerCode=" + partnerCode + "&accessKey=" + accessKey + "&requestId=" + transId + "&amount=" + amount + "&orderId=" + orderId + "&orderInfo=Test&orderType=momo_wallet&transId=" + transId + "&message=" + message + "&localMessage=VN&responseTime=" + transId + "&errorCode=" + errorCode + "&payType=qr&extraData=" + extraData;
        ...
        let tUrl = document.getElementById(type === 'return' ? 'redirectUrl' : 'ipnUrl');
        let targetUrl = tUrl ? tUrl.value : 'http://localhost';
    ...
    function updateUrlRow(id, url) {
        ...
        let linkNode = document.getElementById(id + '_link');
        linkNode.href = url;
